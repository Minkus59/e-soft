...
if ((!empty($_GET['id']))&&(isset($_POST['oui']))) {
    ...
    if($Diffusion->diffusion!=1) {
        $Erreur="Impossible de désactiver la diffusion sur cette email";
    }
    else {
        $Update=$cnx->prepare("UPDATE ".$Prefix."neuro_Diffusion_Liste SET diffusion=0 WHERE id=:id");
...
Etes-vous sur de vouloir d�sactiver cette email ? </p>
